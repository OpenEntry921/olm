...
const biostarPngs=["biostar-bathroom-cleaner-700ml.png","biostar-dishwasher-tablets-50pcs.png","biostar-dishwashing-liquid-700ml.png","biostar-glass-and-mirror-cleaner-700ml.png","biostar-kitchen-cleaner-700ml.png","biostar-shower-cabin-cleaner-700ml.png","biostar-toilet-gel-750ml.png","biostar-universal-cleaner-800ml.png"];
...
test("build copies only the approved source PNGs",async()=>{const names=await readdir(new URL("dist/docs/source-images/",root));assert.deepEqual(names.sort(),[
  ...
  "ChatGPT Image 2026년 8월 22일 오후 02_15_58.png", ...biostarPngs
].sort())});
test("brand detail pages render approved product assets",async()=>{const ludwik=await readFile(new URL("brands/ludwik/index.html",root),"utf8");const biostar=await readFile(new URL("brands/biostar/index.html",root),"utf8");assert.match(ludwik,/src="\/docs\/source-images\/Ludwik-logo\.png"/);assert.equal((ludwik.match(/class="product-placeholder"/g)||[]).length,4);assert.equal((biostar.match(/data-product-id=/g)||[]).length,8);for(const name of biostarPngs)assert.match(biostar,new RegExp(`src="/docs/source-images/${name.replaceAll(".","\\.")}"`));assert.doesNotMatch(biostar,/brand-wordmark|<img[^>]+alt="BIOstar"/);for(const html of [ludwik,biostar])assert.doesNotMatch(html,/src="[^"]+\.(?:tif|pdf)"/i)});
test("BIOstar catalogue and unset claims stay data-driven",async()=>{const source=await readFile(new URL("src/content/ko/biostar.js",root),"utf8");assert.equal((source.match(/certifications: \[\]/g)||[]).length,8);assert.equal((source.match(/purchaseUrl: null/g)||[]).length,8);assert.match(source,/TODO: 공식 BIOstar 로고 업로드 후 교체/);assert.match(source,/logo: \{ path: null/);assert.match(source,/officialPurchaseUrl: null/);assert.doesNotMatch(source,/futureImage|imageReady|\.tiff?\b/i)});
