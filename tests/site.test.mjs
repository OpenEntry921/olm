...
test("hero uses the original landing PNG and reduced motion",async()=>{const html=await readFile(new URL("index.html",root),"utf8");assert.match(html,/class="hero-background"/);const css=await readFile(new URL("assets/css/style.css",root),"utf8");assert.ok(css.includes(`/docs/source-images/${landingName}`));assert.match(css,/@media\(prefers-reduced-motion:reduce\)/);assert.match(css,/\.hero-background\{animation:none/)});
