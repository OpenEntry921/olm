...
const footer = `<footer class="footer"><div class="container"><div class="footer-top">${logo}<div class="footer-info"><span><b>${f.companyLabel}</b> ${f.companyName}</span><span><b>${f.emailLabel}</b> ${f.email}</span><span><b>${f.phoneLabel}</b> ${f.phone}</span><span><b>${f.addressLabel}</b> ${f.address}</span></div></div><div class="footer-bottom">${f.copyright} &nbsp; <a href="/contact/">${f.contactLabel}</a></div></div></footer>`;
